...
const contentPagesPost = async (payload) => {
  ...
  const { data: { data: page = {} }, error = false } = await httpClient.post(`/v3/content/pages`, { ...payload, body: '' });
  if (error) {
...
const contentPagesGet = async (query, lang, page) => {
  ...
  const { data: { data } } = await httpClient.get(`/v3/content/pages?${searchParams}`);
